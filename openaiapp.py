...
def send_to_openai(prompt, model, temperature=0):
    try:
...
# Classify query type
def classify_query(user_prompt, modelPreference):
    prompt = f"""
...
"""
    result = send_to_openai(prompt, modelPreference)
    return result.strip().upper() if result else "GENERAL"

# Handle general questions
def answer_general(question, modelPreference, chat_summary=""):
...
{question}
"""
    return send_to_openai(prompt, modelPreference).strip()


# Generate SQL query using metadata
def generate_duckdb_sql_query(user_prompt, table_metadata, modelPreference, chat_summary=""):
...
"""
    return send_to_openai(prompt, modelPreference)

# Natural language summary
def summarize_result_naturally(sql_query, rows, columns, modelPreference):
...
def get_title(summary, modelPreference, user_prompt=""):
    prompt = f"""
...
"""
    title = send_to_openai(prompt, modelPreference)
    return title.strip().strip('"\'') if title else "New Conversation"

# Chat summarizer
def summarize_chat_history(chat_history, modelPreference):
    try:
...
def handle_query():
    ...
    modelPreference = data.get("modelPreference")
    
    try:
        if df.empty:
            return jsonify({"error": "Excel data not loaded."}), 500

        chat_summary = summarize_chat_history(chat_history, modelPreference) if chat_history and not isNewChat else ""

        query_type = classify_query(user_prompt, modelPreference)
        if query_type == "GENERAL":
            response = answer_general(user_prompt, modelPreference, chat_summary)
            title = get_title(response, modelPreference, user_prompt) if isNewChat else ""
            return jsonify({
                "summary": response,
                "title": title,
                "is_sql_query": False,
        ...
        # SQL path
        generated_sql = generate_duckdb_sql_query(user_prompt, table_metadata, modelPreference, chat_summary)
        ...
        summary = summarize_result_naturally(generated_sql, rows, columns, modelPreference)
        title = get_title(summary, modelPreference, user_prompt) if isNewChat else ""
